...
describe("Test de RUTAS", () =>{
    ...
    describe("GET /rickandmorty/login", () => {
        it("retorna objeto {access: true } con credenciales correctas", async () =>{
            ...
            expect(response.body.access).tobe({ access: true})
        })
    })
    describe("GET /rickandmorty/login", () => {
    ...
    describe("DELETE /rickandmorty/fav/:id", () => {
        it("Devuelve array con personaes si no elimina ningun personaje", async() => {
            const response = await agent.delete("/rickandmorty/fav/15")
            expect(response.body), toContaininEqual(character1)
            expect(response.body), toContaininEqual(character2)           
        })
        it("Devuelve array sin personaes del id enviado", async() => {
            const response = await agent.delete("/rickandmorty/fav/15")
            expect(response.body), not.toContaininEqual(character1)
            expect(response.body), toContaininEqual(character2)           
        })
